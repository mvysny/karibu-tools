 ...
 */
public fun <T> Grid<T>.refreshItem(item: T): Unit = dataProvider.refreshItem(item)
 ...
 */
public fun <T> Grid<T>.getColumnBy(property: KProperty1<T, *>): Column<T> =
...
public inline fun <reified T, reified V> Grid<T>.addColumnFor(
    ...
    val column: Column<T> = addColumn { converter(V::class.java.cast(getter.invoke(it))) }
    return column.apply {
        this.key = key
        if (sortable) {
...
get() {
    ...
    val gridColumn: Column<*> = _AbstractColumn_getBottomLevelColumn.invoke(this) as Column<*>
 ...
 */
public val <T> Column<T>.asc: GridSortOrder<T> get() = GridSortOrder(this, SortDirection.ASCENDING)
/**
 * Creates a [GridSortOrder] sorting this column descending.
 */
public val <T> Column<T>.desc: GridSortOrder<T> get() = GridSortOrder(this, SortDirection.DESCENDING)
 ...
 */
public var <T> Column<T>.header2: String
    get() {
        if (VaadinVersion.get.major >= 24) {
...
private val _Column_getInternalId: Method by lazy(LazyThreadSafetyMode.PUBLICATION) {
    val m = Column::class.java.getDeclaredMethod("getInternalId")
    m.isAccessible = true
    m
}
 ...
 */
public val Column<*>._internalId: String
